...
def run_model(args):
    ...
    if train == True:
        logging.info(
            f"=================== {modelname} start training ==================="
        )
        X_train, y_train = dataloader(vectorize_method).load_traindata()
        if model in ["svm", "rf", "logisticReg"]:
            # 网格搜索
            logging.info("gridsearching...")
            clf = GridSearchCV(
                clf_dict[model], tuned_parameters[model], scoring="accuracy", cv=5
            )
            clf.fit(X_train, y_train)
            logging.info(
                "model: {}, best_params: {}".format(modelname, clf.best_params_)
            )
            ...
            logging.info(f"no such model: {model}")
    else:
        logging.info(
            f"=================== {modelname} start testing ==================="
        )
        ...
        logging.info(
            "model: {}, test_acc_mean: {}, test_f1_mean: {}".format(
