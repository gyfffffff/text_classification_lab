...
class dataloader(object):
    def __init__(self, method="tf-idf"):
        self.train_data_path = r"exp1_data/train_data.txt"
        self.puncs = [",", ".", "(", ")", ";", ":", "[", "]", "{", "}"]
        self.labels = []
        self.method = method
    ...
    def load_raw_traindata(self):
        return pickle.load(open("X_train.pkl", "rb")), pickle.load(
            open("y_train.pkl", "rb")
        )

    def load_raw_testdata(self):
        return pickle.load(open("X_test.pkl", "rb")), pickle.load(
            open("y_test.pkl", "rb")
        )
    ...
    def vectorize(self, X):
        if self.method == "tf-idf":
            ...
            try:
                ...
                print('load bert vectorized data from bert_train.pkl. shape: {}\n'.format(X_2.shape))
                return X_2
            except:    
                X_2 = bert(X)
    ...
    def load_traindata(self):
        if self.method == "bert":
            texts, labels = self.get_raw()
            X_train, X_test, y_train, y_test = train_test_split(
                texts, labels, test_size=0.2, random_state=0
            )
            return (self.vectorize(X_train), y_train)
        else:
            return (
                self.vectorize(self.load_raw_traindata()[0]),
                self.load_raw_traindata()[1],
            )
    ...
    def load_testdata(self):
        if self.method == "bert":
            ...
            return (bert(X_test), y_test)
        else:
            return (
                self.vectorize(self.load_raw_testdata()[0]),
                self.load_raw_testdata()[1],
            )
